*!* SELECT  CurArticulos &&Cursor antes
*!* BROWSE

*!* SELECT  CurRes &&Cursor despues
*!* BROWSE
...
SELECT precio_venta, precio_desc FROM init3 INTO CURSOR curInit3

IF !EMPTY(ALLTRIM(curInit3.precio_venta)) THEN
	lcPrecioVenta = ALLTRIM(curInit3.precio_venta)
ELSE
	lcPrecioVenta = 'd'
ENDIF 

IF !EMPTY(ALLTRIM(curInit3.precio_desc)) THEN
	lcPrecioDesc = ALLTRIM(curInit3.precio_desc)
ELSE
	lcPrecioDesc = 'c'
ENDIF 

TEXT TO lcQry NOSHOW TEXT PRETEXT 7 
	SELECT 
		cr.* 
	FROM 
		CurArticulos ca 
	INNER JOIN CurRes cr ON cr.codart = ca.codart 
	INNER JOIN categoria cat ON cr.grupo = cat.grupo 
	INNER JOIN catadicional catad ON cr.grupo4 = catad.grupo4 
	WHERE 
		(cr.ecommerce = .T. OR cr.maki = .T.) 
		AND cr.idexterno > 0 
		AND cr.idupdate > 0 
		AND cr.sync = .T. 
		AND (!EMPTY(cr.grupo) AND cat.sync = .T. AND cat.idexterno > 0) 
		AND (!EMPTY(cr.grupo4) AND cat.sync = .T. AND cat.idexterno > 0) 
	GROUP BY 
		cr.codart 
	HAVING cr.precio_<<lcPrecioVenta>> <> ca.precio_<<lcPrecioVenta>> 
		OR cr.precio_<<lcPrecioDesc>> <> ca.precio_<<lcPrecioDesc>> 
	INTO CURSOR curResp
ENDTEXT
...
FUNCTION subirMaki(lcCodartAux, lcServicioAux)
	...
	*<-----------fin de carga de stock
		...
		WAIT WINDOW "CONSULTANDO ART�CULOS HIJOS" TIMEOUT 1
		SELECT ;
			codart ;
		FROM ;
			articulos_hijos ;
